...
describe("getRuntimeVersion", () => {
  ...
  test("syncs VERSION file when git is available", async () => {
    const readFileSyncMock = jest.fn((filePath) => {
      if (String(filePath).endsWith("package.json")) {
        return JSON.stringify({ version: "2.4.6" });
      }
    ...
    expect(VersionUtils.syncVersionFile()).toStrictEqual("2.4.6+abc1234");
    expect(writeFileSyncMock).toHaveBeenCalledWith(expect.stringContaining("VERSION"), "2.4.6+abc1234\n", "utf8");
  });

  test("throws when syncing VERSION file and git is unavailable", async () => {
    const readFileSyncMock = jest.fn((filePath) => {
      if (String(filePath).endsWith("package.json")) {
        return JSON.stringify({ version: "2.4.6" });
      }
    ...
    expect(() => VersionUtils.syncVersionFile()).toThrow(
      "Unable to synchronize VERSION: git short SHA is unavailable.",
    );
